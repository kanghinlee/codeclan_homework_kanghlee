...
valid_rmse <- c(mean(kfold_manual_model$resample$RMSE), mean(kfold_exhaustive_model$resample$RMSE),mean(kfold_genetic_model$resample$RMSE) )
valid_rsq <- c(mean(kfold_manual_model$resample$Rsquared), mean(kfold_exhaustive_model$resample$Rsquared),  mean(kfold_genetic_model$resample$Rsquared))
...
region_moda <- lm(average_price ~ region + is_organic + total_volume + season, data = region_log_avocado)

anova(region_null_mod, region_moda)
summary(region_moda)
autoplot(region_moda)
```
